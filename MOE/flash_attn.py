...
def _attn_bwd_dq(
    ...
    for blk_idx in range(num_steps):
        ...
        dQ_block += softmax_scale * tl.dot(dS_block, tl.trans(K_T_block.to(tl.float16)))
        # Increment pointers.
        curr_kv += BLOCK_KV
...
def _attn_bwd_dk_dv(
    ...
    ).to(tl.float16)  # Shape: (BLOCK_KV1, HEAD_DIM)
    ...
    for blk_idx in range(num_steps):
        ...
        qT_block = qT_block.to(tl.float16)
...
#     test_op(BATCH_SIZE=8, NUM_HEADS=8, SEQ_LEN=128, HEAD_DIM=64, causal=True)
#     test_op(BATCH_SIZE=8, NUM_HEADS=8, SEQ_LEN=128, HEAD_DIM=64, causal=False)
#     print("PASSED")